...
{  
    ...
    // array<U64, 64> rook_magics{};
    // array<vector<U64>, 64> rook_attacks_table;

    // array<U64, 64> bishop_magics{};
    // array<vector<U64>, 64> bishop_attacks_table;

    // for (int square = 0; square < 64; ++square) {
    //     int bits = ROOK_RELEVANT_BITS[square];
    //     int table_size = 1 << bits;
    //     rook_attacks_table[square].resize(table_size);
    //     rook_magics[square] = find_magic(square, bits, rook_attacks_table[square].data(), rook_magics[square]);

    //     cout << "Square " << square << ": Magic = 0x" << hex << rook_magics[square] << dec << "\n";
    // }

    // --- Save to file ---
    // ofstream file("rook_magic_data.txt");
    // file << "const U64 rook_masks[64] = {\n";
    // for (int i = 0; i < 64; ++i) {
    //     file << "  0x" << hex << rook_masks[i] << (i != 63 ? ",\n" : "\n");
    // }
    // file << "};\n\n";

    // file << "const U64 rook_magics[64] = {\n";
    // for (int i = 0; i < 64; ++i) {
    //     file << "  0x" << hex << rook_magics[i] << (i != 63 ? ",\n" : "\n");
    // }
    // file << "};\n\n";

    // file << "const U64 rook_attacks[64][4096] = {\n";
    // for (int square = 0; square < 64; ++square) {
    //     file << "  {\n    ";
    //     for (int i = 0; i < 4096; ++i) {
    //         file << "0x" << hex << rook_attacks[square][i];
    //         if (i != 4095) file << ", ";
    //         if ((i + 1) % 4 == 0) file << "\n    ";
    //     }
    //     file << "},\n";
    ...
    // cout << "Magic numbers and attack tables saved to rook_magic_data.txt\n";

    //     // --- Save to file ---
    //     ofstream file2("bishop_magic_data.txt");
    //     file2 << "const U64 bishop_masks[64] = {\n";
    //     for (int i = 0; i < 64; ++i) {
    //         file2 << "  0x" << hex << bishop_masks[i] << (i != 63 ? ",\n" : "\n");
    //     }
    //     file2 << "};\n\n";
    
    //     file2 << "const U64 bishop_magics[64] = {\n";
    //     for (int i = 0; i < 64; ++i) {
    //         file2 << "  0x" << hex << bishop_magics[i] << (i != 63 ? ",\n" : "\n");
    //     }
    //     file2 << "};\n\n";
    
    //     file2 << "const U64 bishop_attacks[64][512] = {\n";
    //     for (int square = 0; square < 64; ++square) {
    //         file2 << "  {\n    ";
    //         for (int i = 0; i < 512; ++i) {
    //             file2 << "0x" << hex << bishop_attacks[square][i];
    //             if (i != 511) file2 << ", ";
    //             if ((i + 1) % 4 == 0) file2 << "\n    ";
    //         }
    //         file2 << "},\n";
    ...
    //     cout << "Magic numbers and attack tables saved to bishop_magic_data.txt\n";
    return 0;
}