...
void initKingAttacks() {
  for (int square = 0; square < 64; ++square) {
    ...
    int f = square % 8;
    if (r > 0)
      kingAttacks[square] |= (1ULL << (square - 8));
    if (r < 7)
      kingAttacks[square] |= (1ULL << (square + 8));
    if (f > 0)
      kingAttacks[square] |= (1ULL << (square - 1));
    if (f < 7)
      kingAttacks[square] |= (1ULL << (square + 1));
    if (r > 0 && f > 0)
      kingAttacks[square] |= (1ULL << (square - 9));
    if (r > 0 && f < 7)
      kingAttacks[square] |= (1ULL << (square - 7));
    if (r < 7 && f > 0)
      kingAttacks[square] |= (1ULL << (square + 7));
    if (r < 7 && f < 7)
      kingAttacks[square] |= (1ULL << (square + 9));
...
void initPawnAttacks() {
  for (int square = 0; square < 64; ++square) {
    ...
    if (r < 7) {
      if (f > 0)
        whitePawnAttacks[square] |= (bitboard << 7);
      if (f < 7)
        whitePawnAttacks[square] |= (bitboard << 9);
    }

    // Black pawn attacks (forward = down the board)
    if (r > 0) {
      if (f > 0)
        blackPawnAttacks[square] |= (bitboard >> 9);
      if (f < 7)
        blackPawnAttacks[square] |= (bitboard >> 7);
    }
  }
}
...
void makemove(Position &pos, Move m) {
  ...
  if (pos.whiteToMove) {
    ...
    if (undo.movedPiece == W_PAWN)
      pos.WhitePawns &= ~from_bb, pos.WhitePawns |= to_bb;
    else if (undo.movedPiece == W_KNIGHT)
      pos.WhiteKnights &= ~from_bb, pos.WhiteKnights |= to_bb;
    else if (undo.movedPiece == W_BISHOP)
      pos.WhiteBishops &= ~from_bb, pos.WhiteBishops |= to_bb;
    else if (undo.movedPiece == W_ROOK)
      pos.WhiteRooks &= ~from_bb, pos.WhiteRooks |= to_bb;
    else if (undo.movedPiece == W_QUEEN)
      pos.WhiteQueen &= ~from_bb, pos.WhiteQueen |= to_bb;
    else if (undo.movedPiece == W_KING)
      pos.WhiteKing &= ~from_bb, pos.WhiteKing |= to_bb;
  } else {
    pos.BlackoccupiedSquares &= ~from_bb;
    pos.BlackoccupiedSquares |= to_bb;
    if (undo.movedPiece == B_PAWN)
      pos.BlackPawns &= ~from_bb, pos.BlackPawns |= to_bb;
    else if (undo.movedPiece == B_KNIGHT)
      pos.BlackKnights &= ~from_bb, pos.BlackKnights |= to_bb;
    else if (undo.movedPiece == B_BISHOP)
      pos.BlackBishops &= ~from_bb, pos.BlackBishops |= to_bb;
    else if (undo.movedPiece == B_ROOK)
      pos.BlackRooks &= ~from_bb, pos.BlackRooks |= to_bb;
    else if (undo.movedPiece == B_QUEEN)
      pos.BlackQueen &= ~from_bb, pos.BlackQueen |= to_bb;
    else if (undo.movedPiece == B_KING)
      pos.BlackKing &= ~from_bb, pos.BlackKing |= to_bb;
  }
  ...
  if (undo.oldCapturedPiece != NO_PIECE) {
    ...
    case W_PAWN:
      ...
      break;
    case W_KNIGHT:
      pos.WhiteKnights &= ~to_bb;
      break;
    case W_BISHOP:
      pos.WhiteBishops &= ~to_bb;
      break;
    case W_ROOK:
      pos.WhiteRooks &= ~to_bb;
      break;
    case W_QUEEN:
      pos.WhiteQueen &= ~to_bb;
      break;
    case W_KING:
      pos.WhiteKing &= ~to_bb;
      break;
    case B_PAWN:
      pos.BlackPawns &= ~to_bb;
      break;
    case B_KNIGHT:
      pos.BlackKnights &= ~to_bb;
      break;
    case B_BISHOP:
      pos.BlackBishops &= ~to_bb;
      break;
    case B_ROOK:
      pos.BlackRooks &= ~to_bb;
      break;
    case B_QUEEN:
      pos.BlackQueen &= ~to_bb;
      break;
    case B_KING:
      pos.BlackKing &= ~to_bb;
      break;
  ...
  if (get_piece_type(undo.movedPiece) == PAWN) {
    ...
    if (std::abs(m.from - m.to) == 16) {
      ...
      pos.enPassant =
          1ULL << ep_square; // enPassant is a bitboard, not a square index!
    ...
    if (m.promotion != NO_PIECE) {
      ...
      if (pos.whiteToMove) {
        if (m.promotion == W_QUEEN)
          pos.WhiteQueen |= to_bb;
        else if (m.promotion == W_ROOK)
          pos.WhiteRooks |= to_bb;
        else if (m.promotion == W_BISHOP)
          pos.WhiteBishops |= to_bb;
        else if (m.promotion == W_KNIGHT)
          pos.WhiteKnights |= to_bb;
      } else {
        if (m.promotion == B_QUEEN)
          pos.BlackQueen |= to_bb;
        else if (m.promotion == B_ROOK)
          pos.BlackRooks |= to_bb;
        else if (m.promotion == B_BISHOP)
          pos.BlackBishops |= to_bb;
        else if (m.promotion == B_KNIGHT)
          pos.BlackKnights |= to_bb;
      }
    }
  }
  ...
  if (get_piece_type(undo.movedPiece) == KING) {
    if (std::abs(m.from - m.to) == 2) { // Castling move
      ...
      if (m.to == 6)
        rook_from = 7, rook_to = 5; // White kingside
      else if (m.to == 2)
        rook_from = 0, rook_to = 3; // White queenside
      else if (m.to == 62)
        rook_from = 63, rook_to = 61; // Black kingside
      else
      ...
      if (pos.whiteToMove) {
        ...
        pos.WhiteoccupiedSquares &= ~rook_from_bb,
            pos.WhiteoccupiedSquares |= rook_to_bb;
      } else {
        pos.BlackRooks &= ~rook_from_bb, pos.BlackRooks |= rook_to_bb;
        pos.BlackoccupiedSquares &= ~rook_from_bb,
            pos.BlackoccupiedSquares |= rook_to_bb;
      }
      pos.zobrist_key ^= zkey.pieceKeys[rook_piece][rook_from];
      pos.zobrist_key ^= zkey.pieceKeys[rook_piece][rook_to];
    }
    if (pos.whiteToMove)
      pos.castelingRights &= ~(WK | WQ);
    else
      pos.castelingRights &= ~(BK | BQ);
  }

  // Update castling rights for rook moves
  if (undo.movedPiece == W_ROOK) {
    if (m.from == 0)
      pos.castelingRights &= ~WQ;
    if (m.from == 7)
      pos.castelingRights &= ~WK;
  }
  if (undo.movedPiece == B_ROOK) {
    if (m.from == 56)
      pos.castelingRights &= ~BQ;
    if (m.from == 63)
      pos.castelingRights &= ~BK;
  }
...
bool is_square_attacked(const Position &pos, int square, bool by_white) {
  ...
  if (by_white) {
    if (GetPawnAttacks(pos, square, false) & pos.WhitePawns)
      return true;
    if (GetKnightAttacks(pos, square) & pos.WhiteKnights)
      return true;
    if (get_bishop_attacks(square, pos.occupiedSquares) &
        (pos.WhiteBishops | pos.WhiteQueen))
      return true;
    if (get_rook_attacks(square, pos.occupiedSquares) &
        (pos.WhiteRooks | pos.WhiteQueen))
      return true;
    // Check if white king attacks this square
    int wking_sq = __builtin_ctzll(pos.WhiteKing);
    if (kingAttacks[wking_sq] & square_bb)
      return true;
  } else {
    if (GetPawnAttacks(pos, square, true) & pos.BlackPawns)
      return true;
    if (GetKnightAttacks(pos, square) & pos.BlackKnights)
      return true;
    if (get_bishop_attacks(square, pos.occupiedSquares) &
        (pos.BlackBishops | pos.BlackQueen))
      return true;
    if (get_rook_attacks(square, pos.occupiedSquares) &
        (pos.BlackRooks | pos.BlackQueen))
      return true;
    // Check if black king attacks this square
    int bking_sq = __builtin_ctzll(pos.BlackKing);
    if (kingAttacks[bking_sq] & square_bb)
      return true;
  }
  return false;
}
...
uint64_t GetQueenMoves(const Position &pos, int square) {
  return get_bishop_attacks(square, pos.occupiedSquares) |
         get_rook_attacks(square, pos.occupiedSquares);
}

uint64_t GetKingMoves(const Position &pos) {
  int square = pos.whiteToMove ? __builtin_ctzll(pos.WhiteKing)
                               : __builtin_ctzll(pos.BlackKing);
...
void generate_captures(Position &pos, MoveList &move_list) {
  ...
  if (pos.whiteToMove) {
    ...
    while (pieces) {
      int from = __builtin_ctzll(pieces);
      uint64_t moves =
          get_bishop_attacks(from, pos.occupiedSquares) & opponent_pieces;
      while (moves) {
        int to = __builtin_ctzll(moves);
        if (move_list.count < 255) {
          move_list.moves[move_list.count++] = {from, to};
    ...
    while (pieces) {
      int from = __builtin_ctzll(pieces);
      uint64_t moves =
          get_rook_attacks(from, pos.occupiedSquares) & opponent_pieces;
      while (moves) {
        int to = __builtin_ctzll(moves);
        if (move_list.count < 255) {
          move_list.moves[move_list.count++] = {from, to};
    ...
    while (pieces) {
      int from = __builtin_ctzll(pieces);
      uint64_t moves = (get_rook_attacks(from, pos.occupiedSquares) |
                        get_bishop_attacks(from, pos.occupiedSquares)) &
                       opponent_pieces;
      while (moves) {
        int to = __builtin_ctzll(moves);
        if (move_list.count < 255) {
...
void generate_quiet_moves(Position &pos, MoveList &move_list) {
  ...
  if (pos.whiteToMove) {
    ...
    while (pieces) {
      int from = __builtin_ctzll(pieces);
      uint64_t moves = GetPawnMoves(pos, from, true) & empty_or_opponent &
    ...
    while (pieces) {
      int from = __builtin_ctzll(pieces);
      uint64_t moves = GetKnightAttacks(pos, from) & empty_or_opponent &
                       ~pos.BlackoccupiedSquares;
      while (moves) {
        int to = __builtin_ctzll(moves);
        if (move_list.count < 255) {
    ...
    while (pieces) {
      int from = __builtin_ctzll(pieces);
      uint64_t moves = get_bishop_attacks(from, pos.occupiedSquares) &
                       empty_or_opponent & ~pos.BlackoccupiedSquares;
      while (moves) {
        int to = __builtin_ctzll(moves);
        if (move_list.count < 255) {
          move_list.moves[move_list.count++] = {from, to};
    ...
    while (pieces) {
      int from = __builtin_ctzll(pieces);
      uint64_t moves = GetPawnMoves(pos, from, false) & empty_or_opponent &
    ...
    while (pieces) {
      int from = __builtin_ctzll(pieces);
      uint64_t moves = get_bishop_attacks(from, pos.occupiedSquares) &
                       empty_or_opponent & ~pos.WhiteoccupiedSquares;
      while (moves) {
        int to = __builtin_ctzll(moves);
        if (move_list.count < 255) {
          move_list.moves[move_list.count++] = {from, to};
...
void check_move_balance() {
  if (make_count != unmake_count) {
    std::cerr << "WARNING: Unbalanced make/unmake! make=" << make_count
              << " unmake=" << unmake_count << std::endl;
  }
}
